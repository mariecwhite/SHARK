...
class EfficientNetB0Module(tf.Module):
    ...
    @tf.function(
        input_signature=[tf.TensorSpec(EFFICIENTNET_B0_INPUT_SHAPE, tf.float32)],
        jit_compile=True,
    )
    def forward(self, inputs):
        return self.m.predict(inputs)
...
class EfficientNetB7Module(tf.Module):
    ...
    @tf.function(
        input_signature=[tf.TensorSpec(EFFICIENTNET_B7_INPUT_SHAPE, tf.float32)],
        jit_compile=True,
    )
    def forward(self, inputs):
        return self.m.predict(inputs)
...
class EfficientNetV2SModule(tf.Module):
    ...
    @tf.function(
        input_signature=[tf.TensorSpec(EFFICIENTNET_V2_S_INPUT_SHAPE, tf.float32)],
        jit_compile=True,
    )
    def forward(self, inputs):
        return self.m.predict(inputs)
