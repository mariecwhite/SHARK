...
# Currently available for macOS m1&intel (3.10) and Linux(3.8,3.10,3.10)
...
if [[ $(uname -s) = 'Darwin' ]]; then
  ...
  if [ "$PYTHON_VERSION_X_Y" == "3.8" ]  || [ "$PYTHON_VERSION_X_Y" == "3.10" ] || [ "$PYTHON_VERSION_X_Y" == "3.10" ] ; then
    torch_mlir_bin=true
  fi
else
...
if [ "$torch_mlir_bin" = true ]; then
  ...
  echo "${Yello}Python 3.10 supported on macOS and 3.8,3.10 and 3.10 on Linux"
